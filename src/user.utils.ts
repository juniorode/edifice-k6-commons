...
export const getHeaders = function(session: Session): { [name: string]: string; } {
  let headers;
  if(session) {
...
export const authenticateWeb = function(login: string, pwd: string) {
  ...
  const cookies: Cookie[] = Object.keys(response.cookies).map(cookieName => {return {name: cookieName, value: response.cookies[cookieName][0].value}});
  return new Session(response.cookies['oneSessionId'][0].value, SessionMode.COOKIE, THIRTY_MINUTES_IN_SECONDS, cookies);
}

export const authenticateOAuth2 = function(login: string, pwd: string, clientId: string, clientSecret: string){
  let credentials = {
    'grant_type': 'password',
    'username': login,
    'password': pwd,
  ...
  return new Session(accessToken, SessionMode.OAUTH2, <number>response.json("expires_in"));
}
