...
export function getRoleByName(name: string, session: Session): Role {
    let roles = http.get(`${rootUrl}/appregistry/roles`, {headers: getHeaders(session)})
    const result = JSON.parse(<string>roles.body);
    return result.filter((role: {name: string}) => role.name === name)[0]
  }


  export function createAndSetRole(applicationName: string, session: Session): Role {
    ...
    if(role) {
      ...
      const application = JSON.parse(<string>res.body).filter((entry: {name: string}) => entry.name === applicationName)[0];
...
export function getRolesOfStructure(structureId: string, session: Session): RoleOfStructure[] {
